...
* descartar stopwords (como “de”, “para”, “com”);
* descartar pontuação e tokens que não são palavras (números, símbolos);
* aplicar lematização com `token.lemma_.lower()` no restante.

> Optamos por **não** filtrar por classe gramatical (POS). As palavras funcionais já são stopwords, então o filtro de POS seria redundante para elas — e tinha o efeito colateral de descartar siglas de domínio que o etiquetador erra (ex.: “fps” marcado como preposição). Remover o filtro deixa o pipeline mais simples e mais robusto.
...
5. **Recomendam-se os jogos desse vizinho** ao usuário novo (ordenados pela relevância textual com a frase digitada). Em outras palavras, o usuário novo **herda** as arestas-jogo do vizinho mais parecido — e é assim que surge a aresta que liga o usuário novo ao grafo. Se o vizinho mais parecido tiver poucos jogos para atingir a quantidade pedida, completa-se a lista com os jogos dos **próximos usuários mais parecidos** (sem repetir).
